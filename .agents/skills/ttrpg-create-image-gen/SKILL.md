...
One implementation (`.agents/cli/image-gen`), two front ends built from the same
parameter spec:

| Surface | How to call it | Available in |
|---|---|---|
| `generate_image` tool | typed object, no shell quoting | pi only |
| `.agents/bin/generate-image` | shell command | every harness, and a bare shell |

Use the native tool if your harness has it; otherwise use the CLI. Parameter
names map to flags in kebab-case (`outputFormat` → `--output-format`,
`dryRun` → `--dry-run`).

```bash
.agents/bin/generate-image --subject "<prompt>"
```
...
```bash
.agents/bin/generate-image --subject "<prompt>" --dry-run
```
...
When using the CLI, quote the prompt as a single argument — it is long prose and
will otherwise be split into positional junk.
...
The same two calls on the CLI:

```bash
.agents/bin/generate-image --size 1536x1024 --quality auto \
  --subject "Draw an original fantasy wide establishing shot of a ruined bell tower rising from a misty salt marsh, cinematic concept art, dawn light, no text, no watermark."

.agents/bin/generate-image --model gpt-image-2 --quality high --size 1024x1024 \
  --subject "Create a player-facing fantasy handout: a weathered silver locket on black velvet, engraved with an abstract moth sigil, photorealistic prop, no extra text, no watermark."
```
...
6. Run `.agents/bin/qmd update` after significant vault edits when search freshness matters.
