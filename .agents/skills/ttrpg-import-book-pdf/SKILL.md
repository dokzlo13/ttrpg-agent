...
description: |
  ...
  book. Wraps the .agents/cli/book-ingest CLI.
---
...
5. Final `.agents/bin/qmd update && .agents/bin/qmd embed` is always emitted last.
...
|---|---|
| ✅ | `classify_system`, `summarize --long-only`, `tag_book`, `.agents/bin/qmd update && .agents/bin/qmd embed` |
| ❌ | `.agents/bin/qmd update && .agents/bin/qmd embed` only |
...
`.agents/env.sh` keeps Marker/qmd/HuggingFace/torch/uv/npm caches under project-local `.cache/`; the qmd index is at `.cache/xdg/qmd/index.sqlite` and its collection config at `.cache/index/index.yml`.
...
```bash
.agents/bin/book-ingest validate \
...
- **LLM mode requested but no key**: add `OPENAI_API_KEY` or rerun with
  `--llm no`.
- **.agents/bin/qmd embed CUDA OOM**: retry with `QMD_LLAMA_GPU=false .agents/bin/qmd embed`.

## Reference

Full CLI contract, flags, JSON shape, env variables, and report fields:
`.agents/cli/book-ingest/README.md`.
