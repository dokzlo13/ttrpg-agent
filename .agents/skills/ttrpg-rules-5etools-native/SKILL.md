...
description: |
  ...
  that require direct reads from .cache/vendor/5etools/.
---
...
- You need a one-off extraction or transformation from `.cache/vendor/5etools/`.
...
- `.cache/vendor/5etools/` is a **read-only clone**.
...
const root = path.resolve(process.env.TTRPG_5ETOOLS_DIR || '.cache/vendor/5etools');
