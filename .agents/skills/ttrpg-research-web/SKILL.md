...
This skill is about **when and how** to go to the web, and how to bring results
back into the vault. The mechanism is whatever your harness provides:

| Harness | Web capability |
|---|---|
| pi | the `pi-web-access` package — `web_search`, `fetch_content` |
| Claude Code | built-in web search and fetch tools |
| Codex | built-in web search when enabled, else an MCP search server |

Use your harness's native search/fetch capability. If it has none, say so
plainly rather than guessing at an answer from memory — an uncited web claim is
worse than "I couldn't search".

Optional API keys (`EXA_API_KEY`, `PERPLEXITY_API_KEY`, `GEMINI_API_KEY`) live in
project `.env` and are read by `.agents/env.sh`; see README prereqs.
...
- Statblocks of canonical 5e creatures → `ttrpg-rules-5etools-query` (`query_5etools` tool, or `.agents/bin/query-5etools`).
