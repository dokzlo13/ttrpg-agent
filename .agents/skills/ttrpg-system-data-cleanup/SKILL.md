...
description: |
  Destructive cleanup/reset procedures for the gitignored TTRPG workspace data: qmd indexes, vault notes, ingested book output, and imports. Use when the user asks to clean up, wipe, reset, purge, or remove vault/import/index data. Requires explicit scope selection, dry-run inventory, and confirmation before deleting anything. Discriminates by sub-path inside .cache: the qmd index is deletable, but the 5.5 GB of model weights beside it, the pinned vendor checkouts, and .agents/state are not. Never touches .agents (the canonical project toolchain), .pi, .claude, repo files, or Obsidian settings.
---
...
| `full-data-reset` | `vault-content` + `imports-all` + `all-index-caches` | Preserves all backbone folders/settings, every model store, and all vendored checkouts. Requires especially clear confirmation. |
...
| `custom-paths` | Only explicitly listed paths under allowed data roots | Refuse anything outside the allowed roots below, and refuse the expensive class **even when explicitly listed**. |
...
| **Expensive — must survive every scope** | `.cache/xdg/qmd/models/` (~2.2 GB), `.cache/xdg/datalab/` (~3.3 GB), `.cache/huggingface/`, `.cache/torch/`, `.cache/uv/`, `.cache/npm/` | Never deletable by any named scope. **Refuse even under `custom-paths`.** |
...
except through the explicit `vendor-checkouts` and `tool-environments` scopes.
...
2. **Inventory.** Run non-destructive commands only:
   ...
   ```bash
   du -sh .cache/xdg/qmd/models .cache/xdg/datalab .cache/vendor 2>/dev/null
   ```
...
It does **not** include `vendor-checkouts` or `tool-environments`; those are
never implied by "reset the data".
