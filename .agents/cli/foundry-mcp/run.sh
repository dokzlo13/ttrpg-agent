...
# Only when NOT launched through .agents/bin/foundry-mcp. env.sh has already
# parsed .env, and re-sourcing it here with `set -a` re-exported the user's
# XDG_CACHE_HOME/npm_config_cache from .env and undid the cache isolation —
# after which install.sh's `npm ci` wrote straight into ~/.npm.
if [[ -z "${TTRPG_ROOT:-}" && -f "$PROJECT_ROOT/.env" ]]; then
  set -a
...
# Defaults must match .agents/env.sh. The upstream clone is a pinned vendor
# checkout under the cache root; the credential is durable agent state under
# .agents/state/ so no cleanup scope can destroy it.
UPSTREAM_DIR="${FOUNDRY_MCP_UPSTREAM_DIR:-$PROJECT_ROOT/.cache/vendor/foundry-vtt-mcp}"
CREDENTIALS_PATH="${FOUNDRY_CREDENTIALS:-$PROJECT_ROOT/.agents/state/foundry-credentials.json}"
...
# Quoted: NODE_OPTIONS is re-split by node, so an unquoted path containing a
# space ("/home/me/my repo/...") becomes two bogus options.
