...
def _find_root() -> Path:
    ...
    Evaluated at import time and used to anchor ``_require_inside``, the
    containment guard of a file-copying tool — so a wrong answer silently
    changes what "inside the vault" means. Key it on ``.agents/`` (canonical)
    rather than ``.pi/`` (one harness adapter, absent in a Claude-only or
    Codex-only clone), and prefer ``TTRPG_ROOT`` when a Tier 1 entrypoint set it.
    """
