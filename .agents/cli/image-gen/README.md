...
`.agents/skills/ttrpg-create-image-gen/SKILL.md`. This README covers only the tool
surface and tests.
...
```bash
node --test .agents/cli/image-gen/*.test.js
```
...
- `image-gen.js` is hand-written ESM (no build step, matching
  `.agents/cli/query-5etools/query-5etools.js`).
...
- `findProjectRoot()` honors `TTRPG_ROOT` (used in tests) and otherwise walks
  upward looking for `.agents/cli/image-gen` or a `.git` + `.env.example`
  pair.
