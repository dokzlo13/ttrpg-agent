...
export async function findProjectRoot() {
  ...
  let current = cwd;
  while (true) {
    // Sentinel is the canonical toolchain, not a harness adapter: .pi/ is
    // absent in a Claude-only or Codex-only clone.
    if (await pathExists(path.join(current, ".agents", "cli", "image-gen"))) return current;
    if (
...
export function buildAssetNote({ planned, responseMetadata, promptSource }) {
  ...
  const body = [
    ...
    "## Sources",
    "",
    "- Generated with OpenAI Images API via `.agents/cli/image-gen`.",
    "",
  ].join("\n");
