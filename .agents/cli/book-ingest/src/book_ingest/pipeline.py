...
def _system_tag(text: str) -> str:
    ...
    heuristics. Use `.agents/bin/book-ingest classify-system <slug>` after ingest for the
...
def _next_steps(result: IngestResult, *, api_key_present: bool) -> list[dict[str, Any]]:
    ...
    steps.append({
            "id": "qmd_refresh",
            "required": True,
            "command": ".agents/bin/qmd update && .agents/bin/qmd embed",
        })
    return steps
