...
def find_project_root(start: Path | None = None) -> Path:
    ...
    The canonical toolchain lives in ``.agents/``; ``.pi/`` is only one harness
    adapter and is absent in a Claude-only or Codex-only clone, so it must not
    be the sentinel. Every ``.agents/bin/*`` entrypoint exports ``TTRPG_ROOT``,
    which is authoritative when no explicit ``start`` is given.
    """
    if start is None:
