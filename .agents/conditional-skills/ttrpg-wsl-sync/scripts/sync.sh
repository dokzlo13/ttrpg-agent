...
# Source the project environment contract so qmd uses the project-local config
# and .env is loaded even when this script is run manually outside a harness.
...
require_dir() {
  local label="$1"
  # NOT `local path`: zsh ties lowercase `path` to PATH. Bash-only today, but
  # this file sources env.sh and the pattern must not be copied.
  local dir="$2"
  if [ ! -d "$dir" ]; then
...
run_qmd_refresh() {
  echo
  echo "== qmd refresh =="
  # Deliberately tolerant. This script runs under `set -e`, and `qmd embed` is
  # exactly the command the CUDA->CPU fallback exists for: letting a non-zero
  # exit abort here would kill the script *after* the rsync has landed and
  # *before* verify_sync, with no retry. Report and continue instead.
  if ! qmd update; then
    echo "WARNING: qmd update failed; index may be stale." >&2
    return 0
  fi
  if ! qmd embed; then
    echo "WARNING: qmd embed failed; semantic search may be stale." >&2
    return 0
  fi
}
...
case "$mode" in
  ...
  push-vault)
    ...
    # The Windows mirror root must already exist. Without this, an unmounted
    # /mnt/c (wsl --shutdown, a drvfs hiccup, a .env typo) makes `mkdir -p`
    # create the whole tree on ext4 *under* the mountpoint, shadowing the real
    # mount — and the verify step then cheerfully reports "in sync".
    require_dir "Windows agent dir" "$win_agent"
    mkdir -p "$win_vault"
    if [ "$apply" -eq 1 ]; then
